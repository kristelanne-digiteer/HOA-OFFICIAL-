...
if (!$row || empty($row['masterlist_id'])) {
    echo "<script>
        alert('Homeowner profile not found.');
...
mysqli_query($conn, "
    ...
    VALUES
    ('{$row['masterlist_id']}', '$billing_month', '$fee', 0, '$fee', '$due_date', 'unpaid', '$purpose')
");
...
mysqli_query($conn, "
    ...
    SET status='approved', document_fee='$fee', soa_id='$soa_id'
    WHERE request_id='$id'
");

logActivity($conn, $admin_id, "Approved document request ID #$id ({$row['document_type']})", "Document");
